...
$query = "SELECT t.*, 
         ...
         WHERE t.sale_id = ? AND t.salesperson_id = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("ii", $saleId, $_SESSION['user_id']);
$stmt->execute();
$result = $stmt->get_result();
if ($result->num_rows > 0) {
    $sale = $result->fetch_assoc();
} else {
    // Check if the sale exists but belongs to another salesperson
    $checkQuery = "SELECT sale_id FROM sales_transactions WHERE sale_id = ?";
    ...
    if ($checkResult->num_rows > 0) {
        $_SESSION['error_message'] = "You don't have permission to view this sale.";
    } else {
